...
import { useTablePagination, useTableSort, useTableMode } from '@/stores/table';
...
const ListingTableTest = () => {
  ...
  const [modes, setModes] = useTableMode();
  ...
  useEffect(() => {
    if (router.query.cat) {
      const querySplit = (router.query.cat as string).split('-');
      const catId = querySplit[querySplit.length - 1];
      setCategoryId(catId.toString());
      setModes.setCategoryMode();
    }
  }, [router]);
